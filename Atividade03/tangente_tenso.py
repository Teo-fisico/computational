...
x_train = np.linspace(-1.2, 1.2, 100).reshape(-1, 1)
...
x_test = np.linspace(-1.4, 1.4, 100).reshape(-1, 1)
...
plt.plot(x_test, y_test, label='Verdadeiro tan(x)', linewidth=2)
...
plt.title('Aproximación de tan(x) con FCNN (tanh) - TensorFlow')
plt.xlabel('x')
plt.ylabel('tan(x)')
plt.legend()
plt.grid(True)
